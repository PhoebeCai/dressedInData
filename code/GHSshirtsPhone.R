...
linePlot = function(data, file, stripeWidth = 2, #stripeGap = 0.5, 
  ...
  #LP: FOR CHANGING THE SIZE OF THE SYMBOLS
  #didn't implement this quite right in the code below (weird symbol placement) but close enough for now
  #was 0.5 and 0.25
  symbolConstant<-1.5
  symbolConstantSm<-0.5
  ...
  if(ntime == 1){
    ...
    for (i in 1:nlines){
      ...
      x = rectXleftedge+stripeWidth/2
      y = data$percentile[i] - symbolConstant
      symbols(x, y, circles = symbolConstant*stripeWidth, fg = as.character(data$visitColor[i]), bg = as.character(data$visitColor[i]), add = TRUE, inches = FALSE)
      symbols(x, y, circles = symbolConstantSm*stripeWidth, fg = as.character(data$stripeColorRank[i]), bg = as.character(data$stripeColorRank[i]), add = TRUE, inches = FALSE)  
      #  abline(v= , lwd) #seems like setting line widths accurately with lwd will be tricky, use rectangles instead
      rectXleftedge<-rectXleftedge + stripeWidth + stripeGap
    }
  }
  if (ntime == 2){
    ...
    for (i in seq(1, nlines, 2)){
      ...
      #Draw the rectangle for the first timepoint  
      rect(rectXleftedge, 0, rectXleftedge+stripeWidth, data$percentile[i], col=as.character(data$visitColor[i]), border = NA)
      x1 = rectXleftedge+symbolConstant*stripeWidth
      y1 = data$percentile[i]
      symbols(x1, y1, circles = symbolConstant*stripeWidth, fg = as.character(data$visitColor[i]), bg = as.character(data$visitColor[i]), add = TRUE, inches = FALSE)
      symbols(x1, y1, circles = symbolConstantSm*stripeWidth, fg = as.character(data$stripeColorRank[i]), bg = as.character(data$stripeColorRank[i]), add = TRUE, inches = FALSE)  
      ...
      symbols(x2, y2, circles = symbolConstant*stripeWidth, fg = as.character(data$visitColor[i+1]), bg = as.character(data$visitColor[i+1]), add = TRUE, inches = FALSE)
      symbols(x2, y2, circles = symbolConstantSm*stripeWidth, fg = as.character(data$stripeColorRank[i+1]), bg = as.character(data$stripeColorRank[i+1]), add = TRUE, inches = FALSE)  
...
#LP: just need participant C and flame retardants
for (i in 1:length(partC))  
  linePlot(partC[[i]], paste("Desktop/GHSdataSept2014/plots/phonePlots/partC", i, "phoneLP.jpg", sep=""), sortType="byAmount")


for (i in 1:length(partA))
  linePlot(partA[[i]], paste("Desktop/GHSdataSept2014/plots/phonePlots/partA", i, "O.jpg", sep=""), sortType="byAmount")
for (i in 1:length(partC))  
  linePlot(partC[[i]], paste("Desktop/GHSdataSept2014/plots/phonePlots/partC", i, "O.jpg", sep=""), sortType="byAmount")
for (i in 1:length(partE))  
  linePlot(partE[[i]], paste("Desktop/GHSdataSept2014/plots/phonePlots/partE", i, "O.jpg", sep=""), sortType="byAmount")
for (i in 1:length(partG))  
  linePlot(partG[[i]], paste("Desktop/GHSdataSept2014/plots/phonePlots/partG", i, "O.jpg", sep=""), sortType="byAmount")
for (i in 1:length(partH))  
  linePlot(partH[[i]], paste("Desktop/GHSdataSept2014/plots/phonePlots/partH", i, "O.jpg", sep=""), sortType="byAmount")
for (i in 1:length(partI))  
  linePlot(partI[[i]], paste("Desktop/GHSdataSept2014/plots/phonePlots/partI", i, "O.jpg", sep=""), sortType="byAmount")


for (i in 2:length(partB))  
  linePlot(partB[[i]], paste("Desktop/GHSdataSept2014/plots/phonePlots/partB", i, "O.jpg", sep=""), sortType="byAmountV2")
for (i in 2:length(partJ))  
  linePlot(partJ[[i]], paste("Desktop/GHSdataSept2014/plots/phonePlots/partJ", i, "O.jpg", sep=""), sortType="byAmountV2")
for (i in 2:length(partD))  
  linePlot(partD[[i]], paste("Desktop/GHSdataSept2014/plots/phonePlots/partD", i, "O.jpg", sep=""), sortType="byAmountV2")
for (i in 2:length(partF))  
  linePlot(partF[[i]], paste("Desktop/GHSdataSept2014/plots/phonePlots/partF", i, "O.jpg", sep=""), sortType="byAmountV2")
