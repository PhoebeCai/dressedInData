...
air3.summary<-ddply(air3, c("Analyte","City","VisitType", "Abbreviation"), summarise,
                   ...
                   P95=signif(quantile(conc_ngperm3, p=0.95, na.rm = TRUE),3),
...
air3.summary.chem<-ddply(air3, c("Analyte", "Abbreviation"), summarise,
                    ...
                    P95chem=signif(quantile(conc_ngperm3, p=0.95, na.rm = TRUE),3),
...
urine.summary<-ddply(urine3, c("Analyte", "Abbreviation","City","VisitType"), summarise,
                     ...
                     P95=signif(quantile(conc_ngml, p=0.95, na.rm = TRUE),3),
...
stripePlot<-function(data, file, stripeWidth, stripeGap, colorData, sortType) {
  ...
  #sortType can be byTime, byAmount (not recommended for multiple visit data?)
  #has percentil data
  #xmax is the 
  #Prep the data
...
if (ntime == 2){
  ...
  #Force them to be sorted so matching chemicals are next to each other
  data<-data[order(data$Analyte, data$Visit, -data$percentile), ]
...
for (i in 1:length(partE))  
  stripePlot(partE[[i]], paste("Desktop/GHSdataSept2014/plots/stripePlots/partE", i, ".jpg", sep=""), stripeWidthOneTime, stripeGapOneTime, stripeColorData, "byTime")
for (i in 1:length(partG))  
  stripePlot(partG[[i]], paste("Desktop/GHSdataSept2014/plots/stripePlots/partG", i, ".jpg", sep=""), stripeWidthOneTime, stripeGapOneTime, stripeColorData, "byTime")
for (i in 1:length(partH))  
  stripePlot(partH[[i]], paste("Desktop/GHSdataSept2014/plots/stripePlots/partH", i, ".jpg", sep=""), stripeWidthOneTime, stripeGapOneTime, stripeColorData, "byTime")
for (i in 1:length(partI))  
  stripePlot(partI[[i]], paste("Desktop/GHSdataSept2014/plots/stripePlots/partI", i, ".jpg", sep=""), stripeWidthOneTime, stripeGapOneTime, stripeColorData, "byTime")


for (i in 1:length(partB))  
stripePlot(partB[[i]], paste("Desktop/GHSdataSept2014/plots/stripePlots/partB", i, ".jpg", sep=""), stripeWidthTwoTime, stripeGapTwoTime, stripeColorData, "byTime")
for (i in 1:length(partJ))  
  stripePlot(partJ[[i]], paste("Desktop/GHSdataSept2014/plots/stripePlots/partJ", i, ".jpg", sep=""), stripeWidthTwoTime, stripeGapTwoTime, stripeColorData, "byTime")
for (i in 1:length(partD))  
  stripePlot(partD[[i]], paste("Desktop/GHSdataSept2014/plots/stripePlots/partD", i, ".jpg", sep=""), stripeWidthTwoTime, stripeGapTwoTime, stripeColorData, "byTime")
for (i in 1:length(partF))  
  stripePlot(partF[[i]], paste("Desktop/GHSdataSept2014/plots/stripePlots/partF", i, ".jpg", sep=""), stripeWidthTwoTime, stripeGapTwoTime, stripeColorData, "byTime")
...
#pull in some sample data to start with
partA1<-read.csv("Desktop/GHSdataSept2014/partA1.csv")
partB2<-read.csv("Desktop/GHSdataSept2014/partB2.csv")


#(start jpeg)

#make a blank plot with the right dimensions
linePlot = function(data, file, stripeWidth = 1, stripeGap = 2, stripeGapSm = 1.5, stripeGapLg = 4) {
  ntime<-length(unique(data$Visit))
  plotWidth = 17
  plotHeight = 17
...
#  plot(0, 0, xlim=c(0, 51), ylim=c(0, 101), main = paste(person, chemClass), col="white")
  ...
  if(ntime == 1){
    ...
    for (i in 1:nlines){
      ...
      y = data$percentile[i]
      symbols(x, y, circles = 1.25*stripeWidth, fg = as.character(data$visitColor[i]), bg = as.character(data$visitColor[i]), add = TRUE, inches = FALSE)
      symbols(x, y, circles = 0.5*stripeWidth, fg = as.character(data$stripeColorRank[i]), bg = as.character(data$stripeColorRank[i]), add = TRUE, inches = FALSE)  
      #  abline(v= , lwd) #seems like setting line widths accurately with lwd will be tricky, use rectangles instead
      rectXleftedge<-rectXleftedge + stripeWidth + stripeGap
    }
  }
   if (ntime == 2){
     plot(0, 0, xlim=c(0, 121), ylim=c(0, 101), main = paste(person, chemClass), col="white")
    #***Make paired stripes touch almost!!  make them one stripe basically with big gap to next one
    #Force them to be sorted so matching chemicals are next to each other
    data<-data[order(data$Analyte, data$Visit), ]
      for (i in seq(1, nlines, 2)){
        ...
        symbols(x1, y1, circles = 1.25*stripeWidth, fg = as.character(data$visitColor[i]), bg = as.character(data$visitColor[i]), add = TRUE, inches = FALSE)
        symbols(x1, y1, circles = 0.5*stripeWidth, fg = as.character(data$stripeColorRank[i]), bg = as.character(data$stripeColorRank[i]), add = TRUE, inches = FALSE)  
        ...
        symbols(x2, y2, circles = 1.25*stripeWidth, fg = as.character(data$visitColor[i+1]), bg = as.character(data$visitColor[i+1]), add = TRUE, inches = FALSE)
        symbols(x2, y2, circles = 0.5*stripeWidth, fg = as.character(data$stripeColorRank[i+1]), bg = as.character(data$stripeColorRank[i+1]), add = TRUE, inches = FALSE)  
...
linePlot(partA1, "Desktop/GHSdataSept2014/plots/linePlots/partA1.jpg")
linePlot(partB2, "Desktop/GHSdataSept2014/plots/linePlots/partB2.jpg")

for (i in 1:length(partA))
  linePlot(partA[[i]], paste("Desktop/GHSdataSept2014/plots/linePlots/partA", i, ".jpg", sep=""))
for (i in 1:length(partC))  
  linePlot(partC[[i]], paste("Desktop/GHSdataSept2014/plots/linePlots/partC", i, ".jpg", sep=""))
for (i in 1:length(partE))  
  linePlot(partE[[i]], paste("Desktop/GHSdataSept2014/plots/linePlots/partE", i, ".jpg", sep=""))
for (i in 1:length(partG))  
  linePlot(partG[[i]], paste("Desktop/GHSdataSept2014/plots/linePlots/partG", i, ".jpg", sep=""))
for (i in 1:length(partH))  
  linePlot(partH[[i]], paste("Desktop/GHSdataSept2014/plots/linePlots/partH", i, ".jpg", sep=""))
for (i in 1:length(partI))  
  linePlot(partI[[i]], paste("Desktop/GHSdataSept2014/plots/linePlots/partI", i, ".jpg", sep=""))


for (i in 1:length(partB))  
  linePlot(partB[[i]], paste("Desktop/GHSdataSept2014/plots/linePlots/partB", i, ".jpg", sep=""))
for (i in 1:length(partJ))  
  linePlot(partJ[[i]], paste("Desktop/GHSdataSept2014/plots/linePlots/partJ", i, ".jpg", sep=""))
for (i in 1:length(partD))  
  linePlot(partD[[i]], paste("Desktop/GHSdataSept2014/plots/linePlots/partD", i, ".jpg", sep=""))
for (i in 1:length(partF))  
  linePlot(partF[[i]], paste("Desktop/GHSdataSept2014/plots/linePlots/partF", i, ".jpg", sep=""))


#add circles on top of each rectangle.  the circle should be centered at the max of the rectangle and the same color as the rectangle


#add a small circle inside the circle.  its color is given by its percentile bin
